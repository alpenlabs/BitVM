...
// FIXME: replace this after verifying this change
pub fn compute_c_wii(f: ark_bn254::Fq12) -> (ark_bn254::Fq12, ark_bn254::Fq12) {
    ...
    assert_eq!(f.pow(cofactor_cubic.to_u64_digits()), ark_bn254::Fq12::ONE);
    ...
    if (f * wi).pow(cofactor_cubic.to_u64_digits()) != ark_bn254::Fq12::ONE {
        assert_eq!(
            (f * w * w * w).pow(cofactor_cubic.to_u64_digits()),
            ark_bn254::Fq12::ONE
        );
        wi = w * w * w;
    }
...
pub fn compute_c_wi(f: ark_bn254::Fq12) -> (ark_bn254::Fq12, ark_bn254::Fq12) {
    ...
    // make f is r-th residue, but it's not cubic residue
    assert_eq!(f.pow(h.to_u64_digits()), ark_bn254::Fq12::ONE);
    let is_cubic = f.pow(cofactor_cubic.to_u64_digits()) == ark_bn254::Fq12::ONE;
    if is_cubic {
        return compute_c_wii(f);
    }
