...
use data_store::data_store::DataStore;
...
pub struct BitVMClientPrivateData {
    ...
    // FIXME: what sorcery is this? Should be split into semantically meaningfull types.
...
impl BitVMClient {
    pub async fn new(
        ...
        // FIXME: ugly repetitive code => refactor as a whole or create a macro
    ...
    */
    ...
    async fn get_latest_file_names(
        ...
        if let Err(e) = all_file_names_result {
            Err(e)
        } else {
    ...
    fn merge_secret_nonces(
        ...
        if self
            .private_data
            .secret_nonces
            .contains_key(&self.verifier_context.as_ref().unwrap().verifier_public_key)
        {
