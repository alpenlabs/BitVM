...
pub(crate) fn write_map_to_file(map: &HashMap<u32, Vec<Vec<u8>>>, filename: &str) -> Result<(), Box<dyn Error>> {
...
pub(crate) fn read_map_from_file(filename: &str) -> Result<HashMap<u32, Vec<Vec<u8>>>, Box<dyn Error>> {
...
mod test {
    ...
    use crate::{bn254::{chunk_config::keygen, chunk_taps::{bitcom_precompute_Py, tap_precompute_Py}}, groth16::offchain_checker::compute_c_wi};
    ...
    fn test_read_write_script() {
        ...
        assert_eq!(read.clone().compile().to_bytes(), script.compile().to_bytes());
    }
}