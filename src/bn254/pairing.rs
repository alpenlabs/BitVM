...
mod test {
    ...
    fn bn254_mm(p1: ark_bn254::G1Affine, q1:ark_bn254::G2Affine) -> ark_bn254::fq12::Fq12 {
        let beta_12x = BigUint::from_str(
            "21575463638280843010398324269430826099269044274347216827212613867836435027261",
        )
        .unwrap();
        ...
        let mut t = q1;
        let p = ark_bn254::G1Affine::new_unchecked(-p1.x/p1.y, p1.y.inverse().unwrap());
        for i in (1..ark_bn254::Config::ATE_LOOP_COUNT.len()).rev() {
            let ate_bit = ark_bn254::Config::ATE_LOOP_COUNT[i - 1];
            // square
            f = f * f;

                // println!("G sq ate {:?}  value {:?}", ate_bit, f);
    ...
    fn test_mm() {
        ...
        let q = ark_bn254::G2Affine::rand(rng);
        let d = bn254_mm(p, q);
        let e = Bn254::multi_miller_loop_affine([p], [q]).0;
        println!("a_prepared {:?}", d);
        println!("a_prepared {:?}", e);
    }
