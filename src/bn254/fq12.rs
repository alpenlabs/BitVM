...
impl Fq12 {
    ...
    pub(crate) fn mul_fp6_by_nonresidue_in_place(fe: ark_bn254::Fq6) -> ark_bn254::Fq6 {
        let mut fe = fe.clone();
...
#[cfg(test)]
mod test {
    use crate::bn254::fq12::{self, Fq12};
    use crate::bn254::fq6::{self, Fq6};
    ...
    #[test]
    fn test_get_fq12_hinted_inv_chunked() {
        fn tap_inv0(a: ark_bn254::Fq12) -> (Script, Vec<Hint>) {

            let (s_t1, h_t1) = fq6::Fq6::hinted_square(a.c1);
            let (s_t0, h_t0) = fq6::Fq6::hinted_square(a.c0);
       ...
       fn tap_inv1(t0: ark_bn254::Fq6) -> (Script, Vec<Hint>) {
            let (scr, hts) = Fq6::hinted_inv(t0);
            (scr, hts)
        }

       fn tap_inv2(a: ark_bn254::Fq12, t1: ark_bn254::Fq6) -> (Script, Vec<Hint>) {
        ...
        fn output_of_tap_inv0(
            ...
            let t1 = a.c1 * a.c1;
            let t0 = a.c0 * a.c0;
            let yt1 = fq12::Fq12::mul_fp6_by_nonresidue_in_place(t1);
            let t0 = t0-yt1;     
            t0
        }

        let t0 = output_of_tap_inv0(a);
        let chunk = tap_inv0(a);
        let scr = script!{
