...
pub(crate) struct ScriptItem {
    ...
    pub(crate) elem_type: DataType, // output type
}

pub(crate) fn groth16_config_gen() -> Vec<ScriptItem> {
    let mut r = vec![];
    let r1 = vec!["GP4y", "GP4x", "GP3y", "GP3x", "GP2y", "GP2x"];
    for item in r1 {
        r.push(ScriptItem {
            category: String::from("GrothG1"),
            link_id: String::from(item),
            dependencies: String::new(),
            elem_type: DataType::P256,
        })
    }
    let r2 = vec![
...
fn assign_ids_to_params(item: &ScriptItem, name_to_id: &mut HashMap<String, (u32, bool)>, cpub_ids: &mut usize, cp256: &mut usize, cp160: &mut usize) {
    if item.category == "GrothPubs" {
        name_to_id.insert(
            item.link_id.clone(),
            (*cpub_ids as u32, true), // todo
        );
        *cpub_ids += 1;
    } else {
        if item.elem_type == DataType::P256 {
            name_to_id.insert(
                item.link_id.clone(),
                (*cp256 as u32, true),
            );
            *cp256 += 1;
        } else if item.elem_type == DataType::P160 {
            name_to_id.insert(
                item.link_id.clone(),
                (*cp160 as u32, false),
            );
